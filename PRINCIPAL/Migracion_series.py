...
class Migracion1:
    ...
    def crear_tabla(self):
        try:
            ...
            cursor=self.conexion.cursor()
            sql="Create table if not exists Series (nombre varchar(255),creador varchar(255),calificacion float, genero varchar (255))"
            cursor.execute(sql,)
            cursor.close()
        ...
        if self.conexion:
            try:
                ...
                for index, row in self.df.iterrows():
                    sql= "INSERT INTO Series(nombre,creador,calificacion,genero) VALUES(%s,%s,%s,%s);"
                    valores=(row["nombre"],row["creador"],row["calificacion"],row["genero"])
...
if __name__=="__main__":
    ...
    migracion=Migracion1("localhost","root","jhhkj","cinema_bd",df)
